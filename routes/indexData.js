...
const calculateTMCAndSave = async (date, indexId, giftsList, lifeData) => {
    try {
        if (!lifeData.length) {
        ...
        await Promise.all(giftsList.map(async (gift) => {
            supplyMap[gift.name] = await getUpgradedSupply(gift.name);
        }));
...
const calculateFDVAndSave = async (date, indexId, giftsList, lifeData) => {
    try {
        if (!lifeData.length) {
            console.log(`No LifeChart data found for date: ${date}`);
            return;
